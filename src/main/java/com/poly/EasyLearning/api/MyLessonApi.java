...
@Slf4j
@RequestMapping("/api/v1/my-lesson")
public class MyLessonApi {
    private final LessonService lessonService;
    private final ImageStorageService imageService;

    @GetMapping("/all")
    public ResponseEntity<ResponseObject> getAllLesson(@RequestParam(name = "username") Integer username){
        return ResponseEntity.status(200).body(
                new ResponseObject(
                        "Get all lesson.",
                        200,
                        lessonService.getAllActiveTrueByUser(username)
                )
        );
    }
